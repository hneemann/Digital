    ...
    public void testCounter() throws Exception {
        ...
        Counter out = model.add(new Counter(
                new ElementAttributes()
                        .setBits(8)));
        out.setInputs(ovs(clk, en, clr));

        TestExecuter sc = new TestExecuter(model).setInputs(clk, en, clr).setOutputs(out.getOutputs());
        sc.check(0, 1, 0, 0, 0);
        sc.check(1, 1, 0, 1, 0);
        sc.check(0, 1, 0, 1, 0);
        sc.check(1, 1, 0, 2, 0);
        ...
        sc.check(0, 1, 0, 0, 0);
        for (int i = 1; i <= 255; i++) {
            sc.check(0, 1, 0, i - 1, 0);
            sc.check(1, 1, 0, i, 0);
        }
        sc.check(0, 1, 0, 255, 0);
        sc.check(1, 1, 0, 0, 1);
        sc.check(0, 1, 0, 0, 1);
        sc.check(1, 1, 0, 1, 0);
    }
