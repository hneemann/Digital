...
public final class CustomShapeEditor extends LabelEditor<CustomShapeDescription> {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fc.showOpenDialog(panel) == JFileChooser.APPROVE_OPTION) {
					try {
						...
						new ErrorMessage("Beim Öffnen der SVG Datei ist ein Fehler aufgetreten (constStr)").addCause(ex)
								.show(panel);
						ex.printStackTrace();
					}
				}
	...
	private final class VPanel extends JPanel {
		...
		private Vector getPosition(MouseEvent e) {
			return new Vector((int) ((e.getX() - translateX) / scale), (int) ((e.getY() - translateY) / scale));
		}
