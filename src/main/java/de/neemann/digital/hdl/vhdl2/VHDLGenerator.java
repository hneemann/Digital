...
public class VHDLGenerator implements Closeable {
    ...
    public VHDLGenerator export(Circuit circuit) throws IOException {
        try {
            ...
            // check for VHDL duplicate net names
            for (HDLCircuit hdlCircuit : model) {
                ArrayList<HDLNet> l = hdlCircuit.getNets();
                for (int i = 0; i < l.size(); i++) {
                    final HDLNet n1 = l.get(i);
                    for (int j = i + 1; j < l.size(); j++) {
                        final HDLNet n2 = l.get(j);
                        if (n1.getName().equalsIgnoreCase(n2.getName())) {
                            if (!n1.isUserNamed() && !n2.isUserNamed()) {
                                final HDLException e = new HDLException(Lang.get("err_namesAreNotUnique_N", n1.getName() + "<>" + n2.getName()));
                                e.setOrigin(hdlCircuit.getOrigin());
                                throw e;
                            } else {
                                if (n1.isUserNamed())
                                    n1.setName("s_" + n1.getName());
                                else
                                    n2.setName("s_" + n2.getName());
                            }
                        }
                    }
                }
