...
/**
 * Component which shows the circuit. ToDo: refactoring of repaint logic. Its to
 * complex now. ToDo: class is to large, move the MouseController classes to
 * their own package
 *
 * @author hneemann
 */
public class CircuitComponent extends JComponent implements Circuit.ChangedListener, LibraryListener {
	/**
	 * The delete icon, also used from
	 * {@link de.neemann.digital.gui.components.terminal.TerminalDialog}
	 */
	 ...
	 *            the parent window
	 * @param library
	 *            the library used to edit the attributes of the elements
	 * @param shapeFactory
	 *            the shapeFactory used for copied elements
	 */
	public CircuitComponent(Main parent, ElementLibrary library, ShapeFactory shapeFactory) {
		...
		}.setToolTipProvider(this::getUndoToolTip).setToolTip(Lang.get("menu_undo_tt")).setAcceleratorCTRLplus('Z')
				.enableAcceleratorIn(this);
		...
		}.setToolTipProvider(this::getRedoToolTip).setToolTip(Lang.get("menu_redo_tt")).setAcceleratorCTRLplus('Y')
				.enableAcceleratorIn(this);
				...
				if (!isLocked()) {
					...
					try {
						...
						SwingUtilities.invokeLater(new ErrorMessage(Lang.get("msg_clipboardContainsNoImportableData"))
								.setComponent(CircuitComponent.this));
					}
				}
			}
	...
	private ArrayList<Movable> getSelectedElements(ShapeFactory shapeFactory) {
		...
		if (activeMouseController instanceof MouseControllerSelect) {
			MouseControllerSelect mcs = ((MouseControllerSelect) activeMouseController);
			elements = circuit.getElementsToCopy(Vector.min(mcs.corner1, mcs.corner2),
					Vector.max(mcs.corner1, mcs.corner2), shapeFactory);
	 ...
	 *            the parent component
	 * @param attrList
	 *            the list of keys to edit
	 */
	public void editCircuitAttributes(Component parent, java.util.List<Key> attrList) {
		ElementAttributes modifiedAttributes = new AttributeDialog(parent, attrList, circuit.getAttributes())
				.showDialog();
	 ...
	 *
	 * @param runMode
	 *            true if running, false if editing
	 * @param modelSync
	 *            used to access the running model
	 */
	 ...
	 *            the style to highlight components
	 */
	...
	public void setPartToInsert(VisualElement element) {
		...
		if (point.x < MOUSE_BORDER_LARGE || point.x > getWidth() - MOUSE_BORDER_SMALL || point.y < MOUSE_BORDER_LARGE
	...
	protected void paintComponent(Graphics g) {
		...
		boolean needsNewBuffer = buffer == null || getWidth() != buffer.getWidth() || getHeight() != buffer.getHeight();

		if (needsNewBuffer && !isManualScale)
			fitCircuit();

		final double scaleX = transform.getScaleX();
		if (graphicsHasChanged || needsNewBuffer || highLighted.size() != highlightedPaintedSize) {
	...
	private void enableAntiAlias(Graphics2D gr2) {
		if (antiAlias) {
			...
			gr2.setRenderingHint(RenderingHints.KEY_ALPHA_INTERPOLATION,
	...
	/**
	 * forces a immediately repaint Is called from
	 * {@link de.neemann.digital.gui.GuiModelObserver} if the models data has
	 * changed. Therefore the double buffer is invalidated.
	 */
		...
		try {
			...
			if (list.size() > 0) {
					...
					attributeDialog.addButton(Lang.get("attr_openCircuitLabel"),
							new ToolTipAction(Lang.get("attr_openCircuit")) {
								...
								public void actionPerformed(ActionEvent e) {
									...
									new Main.MainBuilder().setParent(CircuitComponent.this)
											.setFileToOpen(((ElementLibrary.ElementTypeDescriptionCustom) elementType)
													.getFile())
											.setLibrary(library).denyMostFileActions().keepPrefMainFile().openLater();
								}
							}.setToolTip(Lang.get("attr_openCircuit_tt")));

					/**
					 * Change graphics of the circuit
					 * 
					 * @author felix
					 */
				...
				attributeDialog.addButton(new ToolTipAction(Lang.get("attr_help")) {
					...
					public void actionPerformed(ActionEvent actionEvent) {
						try {
							new ElementHelpDialog(attributeDialog, elementType, element.getElementAttributes())
									.setVisible(true);
	...
	public boolean isLocked() {
		...
		if (locked && !lockMessageShown) {
			String message = Lang.get("msg_isLocked", Lang.get("menu_edit"), Lang.get("menu_editAttributes"),
	...
	/**
	 * All fuses (diodes) are restored to "not programed" so that they are working
	 * again.
	 */
	...
	// MouseController can not be final because its overridden. Maybe checkstyle has
	// a bug?
	...
	private final class MouseControllerMoveElement extends MouseController {
		...
		void clicked(MouseEvent e) {
			if (!isLocked()) {
				...
				if (!visualElement.getPos().equals(initialPos) || visualElement.getRotate() != initialRot)
	...
	private final class MouseControllerMoveWire extends MouseController {
		...
		@Override
		public void drawTo(Graphic gr) {
			// ensure that highlighted wire is visible by drawing it on top of other
			// drawings.
			...
			if (e.isControlDown()) {
				...
				if (e.getButton() == MouseEvent.BUTTON3)
					...
					modify(new Modifications.Builder(Lang.get("mod_insertWire"))
							...
							.add(new ModifyInsertWire(wire2).checkIfLenZero()).build());
	...
	private final class MouseControllerWireSplit extends MouseController {
		...
		void clicked(MouseEvent e) {
			if (e.getButton() == MouseEvent.BUTTON1) {
				addModificationAlreadyMade(new ModifySplitWire(origWire, newPosition));
				circuit.elementsMoved();
				mouseNormal.activate();
			} else
				escapePressed();
	...
	private final class MouseControllerSelect extends MouseController {
		...
		private void updateHighlighting() {
			ArrayList<Drawable> elements = circuit.getElementsToHighlight(Vector.min(corner1, corner2),
		...
		void released(MouseEvent e) {
			if (wasMoved) {
				addModificationAlreadyMade(
						new ModifyMoveSelected(min, max, accumulatedDelta, accumulatedRotate, center));
				circuit.elementsMoved();
			}
			removeHighLighted();
			mouseNormal.activate();
	 ...
	 * Activate a wizard
	 *
	 * @param wizardNotification
	 *            the wizard notification
	 */
